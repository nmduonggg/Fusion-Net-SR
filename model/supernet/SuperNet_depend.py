"""
SuperNet_depend mean that the image generated in stage i will be scale down and add to feature before process to i+1
Then i+1 indirectly depends on i
"""
...
class SuperNet_depend(nn.Module):
    def __init__(self, scale: int=2, tile:int=2):
        super(SuperNet_depend, self).__init__()
        ...
        with torch.no_grad():
            for t in range(T):
                ...
                for i in range(4):
                    x, _ = self.body[i](x)
                    im_x = self.tails[i](x)
                    x = F.interpolate(im_x, size=[x.shape[2], x.shape[3]]) + x
                    MC_outs[i].append(im_x.detach().cpu())
