...
class SuperNet_kul(nn.Module):
    ...
    def forward(self, x, use_dropout=False):
        ...
        masks = [self.mask_predictors[i](out) for i, out in enumerate(outs)]
        final_outs = list()
        final_masks = list()
        
        for i, out in enumerate(outs_mean):
            if i>0:
                # prev_outs = outs_mean[i-1]
                prev_masks = masks[i-1]
                h, w = outs[i].shape[2:]
                # prev_outs = prev_outs*self.gumbel_softmax(prev_masks)
                prev_masks = self.gumbel_softmax(F.interpolate(prev_masks, size=[h, w], mode='nearest'))
                # prev_outs = torch.sum(prev_outs, dim=0)
                prev_outs = outs[i-1]*(1-prev_masks) + outs[i]*prev_masks
                lbda = 0.2
                
                final_outs.append(self.tails[i](outs[i]*(1-lbda) + prev_outs*lbda, use_dropout=use_dropout))
                final_masks.append(self.mask_predictors[i](outs[i]*(1-lbda) + prev_outs*lbda, use_dropout=False))
            
            else:
                final_outs.append(self.tails[i](outs[i], use_dropout=use_dropout))  
                final_masks.append(self.mask_predictors[i](outs[i]))
            
        return [final_outs, final_masks]
    
    # def create_MC_std(self, x, T):
    #     feat = self.heads(x)
    ...
    def create_MC_std(self, x, T):
        ...
        bs = x.shape[0]
        x = torch.cat([x]*T, dim=0) # bsxT instances
        
        with torch.no_grad():
            yfs, masks = self.forward(x, use_dropout=True)
        yfs = [yf.view(bs, -1, yf.shape[1], yf.shape[2], yf.shape[3]) for yf in yfs]  # bsTxCxHxW -> bsxTxCxHxW
        MC_std = [torch.std(MC_out, dim=1) for MC_out in yfs]
...
class BasicBlock(nn.Module):
    def __init__(self, channels, tile, scale=2):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(channels, channels//2, 3, 1, 1), nn.ReLU(True),
            nn.Conv2d(channels//2, channels//2, 3, 1, 1), nn.ReLU(True),
            nn.Conv2d(channels//2, channels, 3, 1, 1, 1)
            )
