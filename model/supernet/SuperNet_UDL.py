...
class SuperNet_udl(nn.Module):
    def __init__(self, scale: int=2, tile:int=2):
        ...
        self.mask_predictors = nn.ModuleList([
            UpSampler(32, self.scale) for _ in range(4)])
        # self.mask_refiner = nn.ModuleList([
        #     nn.Conv2d(1, 1, 3, 1, 1) for _ in range(3)
        # ])
        # self.mask_refiner = nn.ModuleList([
        #     nn.Sequential(nn.Conv2d(1, 1, 3, 1, 1), 
        #                   nn.ReLU(),
        #                   nn.Conv2d(1, 1, 1, 1, 0)) for _ in range(3)])
    ...
    def forward(self, x):
        ...
        for i, x in enumerate(outs):
            ...
            if i>0:
                prev_mask = masks[i-1]
                prev_mask = self.gumbel_softmax(F.interpolate(prev_mask, size=[h, w], mode='nearest'))
                prev_out = x*prev_mask + outs[i-1]*(1-prev_mask)
                lbda = 0.0
                
                if i==(len(outs)-1):
                    final_outs.append(self.tails[i]((1-lbda)*x + lbda*prev_out))
                    final_masks.append(self.mask_predictors[i]((1-lbda)*x + lbda*prev_out))
                else:
