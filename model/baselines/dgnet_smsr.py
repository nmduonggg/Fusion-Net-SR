...
class DGNetSMSR(nn.Module):
    def __init__(self, scale: int=2, tile:int=2):
        super(DGNetSMSR, self).__init__()
        ...
        self.tails = nn.Sequential(
            nn.Conv2d(32, 1, 1, 1, 0), UpSampler(32))
        self.density = []
        self.masked_s = []
...
class BasicBlock(nn.Module):
    def __init__(self, channels, tile):
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.Conv2d(channels, channels//2, 3, 1, 1, bias=False), nn.ReLU(),
            nn.Conv2d(channels//2, channels, 3, 1, 1))
        # self.mask_c = ChannelAttention(channels, channels)
        self.mask_c = nn.Parameter(torch.randn(1, channels, 1, 1))
    ...
    def forward(self, x):
        ...
        masked_c = self.gumbel_softmax(self.mask_c)
        masked_s = self.mask_s(x, False)
        round_c = masked_c.round()
        round_s = masked_s.round()
        # masked_s = F.interpolate(masked_s, size=[H, W], mode='nearest')
        for i, conv in enumerate(self.convs):
            x = self.convs[i](x)
            x = x * masked_c + x*(1-masked_c)*masked_s
            x = F.relu(x)
        x = self.final_conv(x) + residual
        x = F.relu(x)
        s = x.contiguous().cpu()
        self.density = (s > 0).float().mean()
        dense_mask = round_c*torch.ones_like(round_s) + (1-round_c) * round_s
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, channels, tile, eps=0.66667,
        ...
        self.atten_s = nn.Sequential(
            nn.Conv2d(channels, channels//2, kernel_size=3, stride=1, bias=bias>=0, padding=1),
            nn.ReLU(),
            nn.Conv2d(channels//2, 3, 1, 1, bias=bias>0)
        )
        # Gate
    ...
    def forward(self, x, pool=True):
        # Pooling
        input_ds = self.avgpool(x) if pool else x
