...
class MGASR(nn.Module):
    def __init__(self, scale: int=2, tile:int=2):
        super(MGASR, self).__init__()
    ...
    def forward(self, x):
        ...
        masks = torch.cat(masks, dim=1).mean(dim=1, keepdim=True)
        x = self.tails(x)
        
        return [x, densities, masks]
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.size()
        residual = x
        masked_c, logit_c  = self.mask_c(x)
        masked_s, logit_s = self.mask_s(x)
        ...
        dense_mask = masked_c * torch.ones_like(masked_s).to(x.device) + (1-masked_c) * masked_s
        
        if not self.training:
            self.masked_s = masked_s
            
        return [x, dense_mask, masked_s]
